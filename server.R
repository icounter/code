...
##########################################################################################################
  observe({
     ...
     updateTextInput(session,"asset_vol",value=paste0(input_data[,which(colnames(input_data)=="vol")],collapse = ","))
     updateTextInput(session,"asset_vol1",value=paste0(input_data[,which(colnames(input_data)=="vol")],collapse = ","))
     updateTextInput(session,"asset_ret1",value=paste0(input_data[,which(colnames(input_data)=="return")],collapse = ","))
     updateTextInput(session,"asset_ret",value=paste0(input_data[,which(colnames(input_data)=="return")],collapse = ","))
     updateTextInput(session,"extreme_stress_loss_scenario1",value=paste0(input_data[,which(colnames(input_data)=="extreme_ret1")],collapse = ","))
     updateTextInput(session,"extreme_stress_loss_scenario2",value=paste0(input_data[,which(colnames(input_data)=="extreme_ret2")],collapse = ","))
     N<-nrow(input_data)
     a=c()
     for( i in 1:(N-1)){
       a<-c(a,as.numeric(input_data[i,(7+i):(6+N)]))
     }
     updateTextInput(session,"asset_corr1",value=paste0(a,collapse = ","))
     updateTextInput(session,"asset_corr",value=paste0(a,collapse = ","))
     updateTextInput(session,"w_now",value=paste0(input_data[,which(colnames(input_data)=="present_position")],collapse = ","))
     updateTextInput(session,"w1",value=paste0(input_data[,which(colnames(input_data)=="present_position")],collapse = ","))
     ...
     bn_extreme1<-as.character(input_data[,which(colnames(input_data)=='BN_extreme1')])
     bn_hidden_extreme1<-as.character(input_data[,which(colnames(input_data)=='BN_hidden_extreme1')])
     BN_extreme1<-paste0(bn_hidden_extreme1[which(nchar(bn_hidden_extreme1)!=0)],collapse = ";")
     BN2_extreme1<-paste0(name[which(nchar(bn_extreme1)!=0)],"+",bn_extreme1[which(nchar(bn_extreme1)!=0)])
     BN_extreme1<-paste0(BN_extreme1,";",paste0(BN2_extreme1,collapse = ";"),";")
     BN_extreme1<-paste0(BN_extreme1,name[which(nchar(bn_extreme1)==0)],collapse = ";")
     updateTextInput(session,"affect_relation_scenario1",value=BN_extreme1)
     con_extreme1=as.character(input_data[,which(colnames(input_data)=='cond_extreme1')])
     con_hidden_extreme1=as.character(input_data[,which(colnames(input_data)=='cond_Hidden_extreme1')])
     condd_extreme1<-paste0(con_hidden_extreme1[which(nchar(con_hidden_extreme1)!=0)],collapse=";")
     condd_extreme1<-paste0(condd_extreme1,";",paste0(con_extreme1[which(nchar(bn_extreme1)!=0)],collapse = ";"),";")
     condd_extreme1<-paste0(condd_extreme1,con_extreme1[which(nchar(bn_extreme1)==0)],collapse = ";")
     updateTextInput(session,"prob_table_scenario1",value=condd_extreme1)
     bn_extreme2<-as.character(input_data[,which(colnames(input_data)=='BN_extreme2')])
     bn_hidden_extreme2<-as.character(input_data[,which(colnames(input_data)=='BN_hidden_extreme2')])
     BN_extreme2<-paste0(bn_hidden_extreme2[which(nchar(bn_hidden_extreme2)!=0)],collapse = ";")
     BN2_extreme2<-paste0(name[which(nchar(bn_extreme2)!=0)],"+",bn_extreme2[which(nchar(bn_extreme2)!=0)])
     BN_extreme2<-paste0(BN_extreme2,";",paste0(BN2_extreme2,collapse = ";"),";")
     BN_extreme2<-paste0(BN_extreme2,name[which(nchar(bn_extreme2)==0)],collapse = ";")
     updateTextInput(session,"affect_relation_scenario2",value=BN_extreme2)
     con_extreme2=as.character(input_data[,which(colnames(input_data)=='cond_extreme2')])
     con_hidden_extreme2=as.character(input_data[,which(colnames(input_data)=='cond_Hidden_extreme2')])
     condd_extreme2<-paste0(con_hidden_extreme2[which(nchar(con_hidden_extreme2)!=0)],collapse=";")
     condd_extreme2<-paste0(condd_extreme2,";",paste0(con_extreme2[which(nchar(bn_extreme2)!=0)],collapse = ";"),";")
     condd_extreme2<-paste0(condd_extreme2,con_extreme2[which(nchar(bn_extreme2)==0)],collapse = ";")
     updateTextInput(session,"prob_table_scenario2",value=condd_extreme2)
       trans_generaed<<-1
...
#####################################################out put rescaled probability matrix
  con_tabe_function_all<-function(pro_dict_scenario1,pro_dict_scenario2,k1,k2){
    pro_dict_scenario1<-data.frame(t(pro_dict_scenario1))
    pro_dict_scenario2<-data.frame(t(pro_dict_scenario2))
    rescale<-rep(0,nrow(pro_dict_scenario1)+nrow(pro_dict_scenario2)-1)
    if((pro_dict_scenario2[1,1]!=1)&&(pro_dict_scenario1[1,1]!=1)){
      rescale[1]<-1-k1-k2
      rescale[2:nrow(pro_dict_scenario1)]<-pro_dict_scenario1[2:nrow(pro_dict_scenario1),1]/(1-pro_dict_scenario1[1,1])*k1
      rescale[(nrow(pro_dict_scenario1)+1):length(rescale)]<-pro_dict_scenario2[2:nrow(pro_dict_scenario2),1]/(1-pro_dict_scenario2[1,1])*k2
    }
    bayesian_matrix1<-rbind(pro_dict_scenario1,pro_dict_scenario2[2:nrow(pro_dict_scenario2),])
  ...
  con_tabe_function<-function(bayesian_matrix,tentative_k){
  bayesian_matrix1<-data.frame(t(bayesian_matrix))
  rescale<-rep(0,nrow(bayesian_matrix1))
  if(bayesian_matrix1[1,1]!=1){
    rescale[1]<-1-tentative_k
    rescale[2:length(rescale)]<-bayesian_matrix1[2:nrow(bayesian_matrix1),1]/(1-bayesian_matrix1[1,1])*tentative_k
  }
  bayesian_matrix1<-cbind(bayesian_matrix1,rescale)
  colnames(bayesian_matrix1)[ncol(bayesian_matrix1)]<-"Rescaled prob"
  bayesian_matrix1<-bayesian_matrix1[,c(c(1,ncol(bayesian_matrix1)),seq(2,ncol(bayesian_matrix1)-1))]
...
con_tabe_scenario1<-eventReactive(input$go_BN_cond,{
   con_tabe_function(pro_dict_scenario1,input$tentative_k[1])
    })
con_tabe_scenario2<-eventReactive(input$go_BN_cond,{
  con_tabe_function(pro_dict_scenario2,input$tentative_k[2]-input$tentative_k[1])
})
con_tabe_scenarioall<-eventReactive(input$go_BN_cond,{
...
generate_bayesian_cor_scenario1<-eventReactive(input$go_BN_cond,{
  generate_bayesian_cor_function(pro_dict_scenario1)
      })
...
generate_extreme_cor_function<-function(pro_dict,extreme_stress_loss,asset_ret1,asset_corr1,asset_vol1){
  pro_dict3<-as.matrix(pro_dict)
  divide<-(pro_dict3[2,pro_dict3[2,]!=0])[1]
...
generate_extreme_cor_scenario1<-eventReactive(input$go_BN_cond,{
generate_extreme_cor_function(pro_dict_scenario1  ,input$extreme_stress_loss_scenario1,input$asset_ret1,input$asset_corr1,input$asset_vol1)
    })
generate_extreme_cor_scenario2<-eventReactive(input$go_BN_cond,{
  generate_extreme_cor_function(pro_dict_scenario2,input$extreme_stress_loss_scenario2,input$asset_ret1,input$asset_corr1,input$asset_vol1)
})
generate_extreme_cor_scenarioall<-eventReactive(input$go_BN_cond,{
...
generate_bayesian_cor2_function<-function(bayesian_matrix1,tentative_k,extreme_stress_loss,asset_ret1,asset_corr1,asset_vol1){
  ...
  if(pro_dict3[1,1]!=1){
    rescale[1]<-1-tentative_k
    rescale[2:length(rescale)]<-pro_dict3[2:nrow(pro_dict3),1]/(1-pro_dict3[1,1])*tentative_k
  }
  pro_dict3[,1]<-rescale
...
generate_bayesian_cor2_scenario1<-eventReactive(input$go_BN_cond,{
  generate_bayesian_cor2_function(pro_dict_scenario1,input$tentative_k[1],input$extreme_stress_loss_scenario1,input$asset_ret1,input$asset_corr1,input$asset_vol1) 
    })
...
generate_normal_cor_scenario1<-eventReactive(input$go_BN_cond,{
  generate_normal_cor_function(input$assets_num1,input$asset_corr1,input$asset_name1) 
    })
generate_normal_cor_scenario2<-eventReactive(input$go_BN_cond,{
  generate_normal_cor_function(input$assets_num1,input$asset_corr1,input$asset_name1) 
})
generate_normal_cor_scenarioall<-eventReactive(input$go_BN_cond,{
  generate_normal_cor_function(input$assets_num1,input$asset_corr1,input$asset_name1) 
})
...
generate_bayesian_cor_downside_scenario1<-eventReactive(input$go_BN_cond,{
  generate_bayesian_cor_downside_function(pro_dict_scenario1,input$extreme_stress_loss_scenario1)
    })
...
#######################################out put data tables
    output$hotable2_scenario1<- DT::renderDataTable(
      DT::datatable(con_tabe_scenario1(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
    )))
    output$bayesian_matrix_cor_normal_scenario1 <- DT::renderDataTable(
      DT::datatable(generate_normal_cor_scenario1(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_extreme_scenario1 <- DT::renderDataTable(
      DT::datatable(generate_extreme_cor_scenario1(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_scenario1 <- DT::renderDataTable(
      DT::datatable(generate_bayesian_cor_scenario1(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_downside_scenario1 <- DT::renderDataTable(
      DT::datatable(generate_bayesian_cor_downside_scenario1(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor2_scenario1 <- DT::renderDataTable(
      DT::datatable(generate_bayesian_cor2_scenario1(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
##################################
    output$hotable2_scenario2<- DT::renderDataTable(
      DT::datatable(con_tabe_scenario2(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_normal_scenario2 <- DT::renderDataTable(
      DT::datatable(generate_normal_cor_scenario2(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_extreme_scenario2 <- DT::renderDataTable(
      DT::datatable(generate_extreme_cor_scenario2(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_scenario2 <- DT::renderDataTable(
      DT::datatable(generate_bayesian_cor_scenario2(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_downside_scenario2 <- DT::renderDataTable(
      DT::datatable(generate_bayesian_cor_downside_scenario2(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor2_scenario2 <- DT::renderDataTable(
      DT::datatable(generate_bayesian_cor2_scenario2(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
#################################
    output$hotable2_scenarioall<- DT::renderDataTable(
      DT::datatable(con_tabe_scenarioall(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_normal_scenarioall <- DT::renderDataTable(
      DT::datatable(generate_normal_cor_scenarioall(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_extreme_scenarioall <- DT::renderDataTable(
      DT::datatable(generate_extreme_cor_scenarioall(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_scenarioall <- DT::renderDataTable(
      DT::datatable(generate_bayesian_cor_scenarioall(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor_downside_scenarioall <- DT::renderDataTable(
      DT::datatable(generate_bayesian_cor_downside_scenarioall(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))
    output$bayesian_matrix_cor2_scenarioall <- DT::renderDataTable(
      DT::datatable(generate_bayesian_cor2_scenarioall(), options = list(
        lengthMenu = list(c(5, 15, -1), c('5', '15', 'All')),
        pageLength = 15
      )))    
#################################plot baeysian network
    plot_bay<-eventReactive(input$go_plot_bayesian_net,{
#       bbb<<-input$affect_relation_scenario2
#       aaa<<-input$prob_table_scenario2
      cond_tab_scenario1=build_cond(affect_relation=input$affect_relation_scenario1,prob_table=input$prob_table_scenario1,yn=c('1','0'))   ###build condition probability table
      plist_scenario1<-compileCPT(cond_tab_scenario1)
      net_scenario1<-grain(plist_scenario1) 
      bn_net_scenario1<-as.bn.fit(net_scenario1)
      asset_name1=strsplit(input$asset_name1,",")[[1]]
      pro_dict_scenario1<<-bayesian_matrix(cond_tab_scenario1,asset_name=asset_name1)
      graphviz.plot(bn_net_scenario1,layout='dot',shape='ellipse')
    })
    output$plot_bayesian<-renderPlot({return(plot_bay())})
    plot_bay2<-eventReactive(input$go_plot_bayesian_net,{
      #       bbb<<-input$affect_relation_scenario2
      #       aaa<<-input$prob_table_scenario2
      cond_tab_scenario2=build_cond(affect_relation=input$affect_relation_scenario2,prob_table=input$prob_table_scenario2,yn=c('2','0')) 
      plist_scenario2<-compileCPT(cond_tab_scenario2)
      net_scenario2<-grain(plist_scenario2) 
      bn_net_scenario2<-as.bn.fit(net_scenario2)
      asset_name1=strsplit(input$asset_name1,",")[[1]]
      pro_dict_scenario2<<-bayesian_matrix(cond_tab_scenario2,asset_name=asset_name1)
      graphviz.plot(bn_net_scenario2,layout='dot',shape='ellipse')
    })
    output$plot_bayesian2<-renderPlot({return(plot_bay2())})
