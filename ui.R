...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      textInput("extreme_stress_loss_scenario1",label = h3("Extreme Loss of each Asset in scenario 1(mild extreme)"), value ="-.2,-0.03,-0.03,0.0065"),
      helpText("Note:This is the return of each assests when extreme event happens"),
      textInput("affect_relation_scenario1", label = h3("Affect Relationship between assets in scenario 1(mild extreme)"), value ="Hidden1;CLO+Hidden1;senior_loan+Hidden1;NPL1stpay+Hidden1;cash"),
      helpText("Define Structure of your BN,semicomma delimited.and A+B means A is affected by B"),
      textInput("prob_table_scenario1", label = h3("Probability table in scenario 1(mild extreme)"), value ="10,90;80,20,10,90;80,20,10,90;80,20,10,90;0,100"),
      helpText("Define conditional probability table for each node,semicomma delimited"),
      textInput("extreme_stress_loss_scenario2",label = h3("Extreme Loss of each Asset in scenario 2(very extreme)"), value ="-.2,-0.03,-0.03,0.0065"),
      textInput("affect_relation_scenario2", label = h3("Affect Relationship between assets in scenario 2(very extreme)"), value ="Hidden1;CLO+Hidden1;senior_loan+Hidden1;NPL1stpay+Hidden1;cash"),
      textInput("prob_table_scenario2", label = h3("Probability table in scenario 2(very extreme)"), value ="10,90;80,20,10,90;80,20,10,90;80,20,10,90;0,100"),
      helpText("outplot the structure of BN at the right panel"),
      actionButton("go_plot_bayesian_net","Generate Bayesian Net plot")
    ),
    mainPanel(titlePanel("bayesian network structure under extreme scenario 1:"),
      plotOutput("plot_bayesian"),hr(),
      titlePanel("bayesian network structure under extreme scenario 2:"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("tentative_k", label = h3("Rescale the probabbility of Bayesian Network"),min = 0.0, max =1.0,step=0.01, value =c(0.01,0.02)),actionButton("go_BN_cond","Show BN conditional Table"),
      selectInput("show_part_or_all", label = h3("show correlation matrix"),choices = list("part" = 'part1', "all" = 'all1'), selected = 'part1')),
    mainPanel(
      tabsetPanel(
        tabPanel('Scenario 1', DT::dataTableOutput("hotable2_scenario1"),      
                 hr(),
                 titlePanel("Normal Scenario Correlation Matrix"),
                 DT::dataTableOutput("bayesian_matrix_cor_normal_scenario1"),  ##normal correaltion matrix
                 hr(),
                 conditionalPanel("input.show_part_or_all=='all1'",             ##conditional panel display when we select to show all
                                  ...
                                  DT::dataTableOutput("bayesian_matrix_cor_extreme_scenario1"),
                                  hr()),
                 titlePanel("Conditional probability Matrix in Extreme Scenario"),
                 DT::dataTableOutput("bayesian_matrix_cor_scenario1"),
                 hr(),
                 conditionalPanel("input.show_part_or_all=='all1'",
                                  titlePanel("Downside correlation in Extreme Scenario"),
                                  DT::dataTableOutput("bayesian_matrix_cor_downside_scenario1"),hr()),
                 titlePanel("Rescaled Correlation Matrix in Bayesian Network"),
                 DT::dataTableOutput("bayesian_matrix_cor2_scenario1")),
        tabPanel('Scenario 2', DT::dataTableOutput("hotable2_scenario2"),
                 hr(),
                 titlePanel("Normal Scenario Correlation Matrix"),
                 DT::dataTableOutput("bayesian_matrix_cor_normal_scenario2"),  ##normal correaltion matrix
                 hr(),
                 conditionalPanel("input.show_part_or_all=='all1'",             ##conditional panel display when we select to show all
                                  titlePanel("Extreme Scenario Correlation Matrix"),
                                  DT::dataTableOutput("bayesian_matrix_cor_extreme_scenario2"),
                                  hr()),
                 titlePanel("Conditional probability Matrix in Extreme Scenario"),
                 DT::dataTableOutput("bayesian_matrix_cor_scenario2"),
                 hr(),
                 conditionalPanel("input.show_part_or_all=='all1'",
                                  titlePanel("Downside correlation in Extreme Scenario"),
                                  DT::dataTableOutput("bayesian_matrix_cor_downside_scenario2"),hr()),
                 titlePanel("Rescaled Correlation Matrix in Bayesian Network"),
                 DT::dataTableOutput("bayesian_matrix_cor2_scenario2")),
         tabPanel('Overal Scenario', DT::dataTableOutput("hotable2_scenarioall"),
                  hr(),
                  titlePanel("Normal Scenario Correlation Matrix"),
                  DT::dataTableOutput("bayesian_matrix_cor_normal_scenarioall"), ##normal correaltion matrix
                  hr(),
                  conditionalPanel("input.show_part_or_all=='all1'",             ##conditional panel display when we select to show all
                                   titlePanel("Extreme Scenario Correlation Matrix"),
